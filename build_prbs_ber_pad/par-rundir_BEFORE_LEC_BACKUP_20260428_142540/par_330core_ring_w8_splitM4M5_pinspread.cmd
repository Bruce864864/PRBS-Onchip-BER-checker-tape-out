...
#  Created on Fri Apr 24 19:20:14 2026                
#                                                     
#######################################################
...
puts "edit_pin left-side clock/reset pins"
edit_pin -fixed_pin -pin {clk rst_n} -hinst prbs_ber_pad_top \
  -pattern fill_optimised -layer { METAL3 METAL5 } \
  -side left -start {334.88 385.00} -end {334.88 455.00}
puts "edit_pin bottom-side cfg pins"
edit_pin -fixed_pin -pin {cfg_sclk cfg_cs_n cfg_mosi cfg_miso} -hinst prbs_ber_pad_top \
  -pattern fill_optimised -layer { METAL2 METAL4 } \
  -side bottom -start {410.00 334.88} -end {530.00 334.88}
puts "edit_pin top-side external serial input"
edit_pin -fixed_pin -pin {ext_serial_in} -hinst prbs_ber_pad_top \
  -pattern fill_optimised -layer { METAL2 METAL4 } \
  -side top -start {500.00 665.28} -end {500.00 665.28}
puts "edit_pin right-side output/status pins"
edit_pin -fixed_pin -pin {busy done prbs_out bit_error} -hinst prbs_ber_pad_top \
  -pattern fill_optimised -layer { METAL3 METAL5 } \
  -side right -start {665.28 405.00} -end {665.28 545.00}
