...
################################################################################
if {[::legacy::find -design design:prbs_ber_pad_top] ne ""} {
  puts "** A design with the same name is already loaded. It will be removed. **"
  delete_obj design:prbs_ber_pad_top
}
...
# slack
set _slk_ [::legacy::get_attribute slack design:prbs_ber_pad_top]
if {[regexp {^-?[0-9.]+$} $_slk_]} {
  set _slk_ [format %.1f $_slk_]
}
if {$_slk_ != "267.5"} {
   mesg_send [::legacy::find -message /messages/PHYS/PHYS-92] "golden slack: 267.5,  current slack: $_slk_"
}
unset _slk_
# multi-mode slack
if {"[string_representation [::legacy::get_attribute slack_by_mode design:prbs_ber_pad_top]]" != "{{mode:prbs_ber_pad_top/WCCOM.setup_view 267.5}}"} {
   mesg_send [::legacy::find -message /messages/PHYS/PHYS-92] "golden slack_by_mode: {{mode:prbs_ber_pad_top/WCCOM.setup_view 267.5}}  current slack_by_mode: [string_representation [::legacy::get_attribute slack_by_mode design:prbs_ber_pad_top]]"
}
# tns
set _tns_ [::legacy::get_attribute tns design:prbs_ber_pad_top]
if {[regexp {^-?[0-9.]+$} $_tns_]} {
...
unset _tns_
# cell area
set _cell_area_ [::legacy::get_attribute cell_area design:prbs_ber_pad_top]
...
unset _cell_area_
# net area
set _net_area_ [::legacy::get_attribute net_area design:prbs_ber_pad_top]
